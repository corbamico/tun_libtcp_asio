      ...
      private asio::detail::noncopyable
{
  ...
  private:
    ...
    void handle_tcp_packet(Tins::IP &ip, Tins::TCP &tcp, channel_index index)
    {
        ...
        {
            //only ACK, we ignore here.
            return;
        }

        //handle tcp packet from here
        Tins::IP response_ip(ip.src_addr(), ip.dst_addr());
        Tins::TCP response_tcp(tcp.sport(), tcp.dport());

        if (tcp.get_flag(Tins::TCP::SYN))
        {
            response_tcp.set_flag(Tins::TCP::SYN, 1);
            response_tcp.set_flag(Tins::TCP::ACK, 1);

            response_tcp.seq(seq++);
            response_tcp.ack_seq(tcp.seq() + 1);
        }
        else if (tcp.get_flag(Tins::TCP::FIN))
        {
            response_tcp.set_flag(Tins::TCP::FIN, 1);
            response_tcp.set_flag(Tins::TCP::ACK, 1);

            response_tcp.seq(seq++);
            response_tcp.ack_seq(tcp.seq() + 1);
        }
        else if (tcp.get_flag(Tins::TCP::PSH))
        {
            response_tcp.set_flag(Tins::TCP::ACK, 1);
            response_tcp.seq(seq);
            response_tcp.ack_seq(tcp.seq() + tcp.inner_pdu()->size());
            auto payload = tcp.inner_pdu()->serialize();
            on_tcp_read(payload);            
        }
    ...
    void on_tcp_read(std::vector<uint8_t> &data)
    {
